...
本仓库的 `aisilun/hermes-agent` 分支只承载 ASL 生产所需、尚未被 Hermes 官方正式版等价覆盖的最小 host capability（宿主能力）。机器可读的唯一发布合同是 [`governance/asl-fork-release.json`](governance/asl-fork-release.json)。

## 当前状态

- package version（包版本）：`0.19.1+asl.1`
- release tag（正式标签）：`v0.19.1-asl.1`（由本次 release-only 合并提交创建）
- production branch（生产分支）：`asl/production`
- source status（源码状态）：`official`（正式）
- 当前 official source version（正式源码版本）：`0.19.1+asl.1`
- 当前正式 Tag：`v0.19.1-asl.1`（由本次 release-only 合并提交创建）
- 当前正式 GitHub Release：`v0.19.1-asl.1`（绑定该 annotated Tag 并完成正文与状态回读）
- review gate（审核门）：`trusted-xiaomu-single-review`，状态 `approved`（已通过）
- merge（合并）：PR #9 已 squash merge 至 `asl/production@aa6fe706adc2a94d09e034e79023210cf2446fce`
- Tag／Release：已授权，仅允许按 Gate 7 创建并回读`v0.19.1-asl.1`
- production activation（生产激活）：未授权
- Fleet apply（全量应用）：未授权

本正式源码允许完成 release-only PR（只发布状态晋级合并请求）、annotated Tag（带注释标签）和 GitHub Release 的对象级创建与回读。它不授权安装到 `default` profile（默认配置档案）、重启 Gateway（网关）、恢复 Cron、执行数据库迁移、production activation 或 Fleet apply。
...
| 上游正式 commit（提交） | `cc4cab2f592e60a197e796506de9168f74baf3ea` | 本正式源码的官方基线 |
...
正式源码不是在 live root（现运行源码树）上执行 `hermes update`，也不是把旧 ASL `.3`整树覆盖到官方 v0.19.1。构建方式是以官方 `cc4cab2…`文件树为基线，使用共同基点完成三方语义重放，再逐项验证官方已覆盖、部分覆盖和未覆盖的能力。

## 最小补丁队列

| 队列项 | 来源 | 官方 v0.19.1 覆盖 | 正式决策 |
|---|---|---|---|
...
- `.lazy-refresh-incomplete`在官方基线中已不再作为跟踪文件；正式源码继续保留 ASL 的 `.gitignore`规则，且不得重新引入该 marker（标记文件）。
...
特权配置档案补丁不是完整 zero-trust broker（零信任授权代理）。同一 OS 用户直接修改 SQLite／进程环境、跨 profile single-use grant（跨配置档案单次授权）和不可伪造 principal binding（主体绑定）仍不属于本正式源码的解决范围；因此任何旧行或绕过写入仍必须在 claim／dispatch／spawn 最终边界再次失败关闭。
...
正式发布或后续隔离验收失败时，源码回滚基线是 `0.19.0+asl.3@d01f138cf889ed95e7b7ff3785b89db55c52e828`及其已记录 Tag／Release。不得通过禁用 fail-closed gate 恢复服务。真实生产回滚命令、配置快照和运行根切换只能在后续 production activation（生产激活）授权包中确定。
